...
6. [recover.py](file:///c:/snorkel/solution/recover.py): Reference Python script that reads the corrupted CSV ledger, computes label counts offline from the pre-cached Hugging Face dataset, reconstructs the missing tree parentage and dataset revision attributes, outputs `recovered_ledger.csv` and `lineage.dot` with deterministic sorting, and compiles them into a Graphviz `lineage.svg` diagram.
